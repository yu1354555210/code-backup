	...
	<view class="comments-box">
		<view class="comments-header">
			...
			<view class="comments-header__info">
				<view v-if="!comment.is_reply" class="title">{{comment.author.author_name}}</view>
				<view v-else class="title">{{comment.author.author_name}} <text class="reply-text">回复</text>{{comment.to}}</view>
		...
		<view class="comments-content">
			...
			<view class="comments-info">
				<view class="comments-button" @click="commentsReply({comments:comment,is_reply:reply})">回复</view>
			</view>
			<view class="comments-reply" v-for="item in comment.replys" :key="item.comment_id">
...
<script>
	...
	export default {
		...
		created() {
			this.comment = this.comments.operands[0];
		},
		data() {
			return {
				comment: {}
