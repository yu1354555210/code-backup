    ...
    <div class="login_popup" v-show="isLoginPopup">
      ...
      <div class="public_popup">
        <div class="public_popup_title flex-pack-justify">
          <h3>
            <i class="this_icon icon_logo"></i>哪吒游戏登录
          </h3>
      ...
      <div class="agreement">注册/登录代表你同意
  ...
  methods: {
    ...
    coopLogin() {
      ...
      function login_jsonpCallback(res) {
        console.log(res);
        $.get(
          "http://h5.nzha.com/xdcoop/player2/login",
          { code: res.code },
          function(res) {}
        );
      }
    ...
    weiboLogin() {
      ...
      if (this.isWeiXin()) {
        ...
        this.$ajax
          ...
          .then(response => {
            localStorage.removeItem("playUrl");
            window.location.href = `${document.location.origin}/weibo-login/index?redirect_url=${encodeURIComponent(
              response.data.data
            )}&ch_id=${this.ch_id}&xch_id=${this.xch_id}`;
          })
          .catch(function(error) {});
      } else {
        window.location.href =
          document.location.origin +
          "/weibo-login/index?redirect_url=" +
          encodeURIComponent(redirect_url);
      }
