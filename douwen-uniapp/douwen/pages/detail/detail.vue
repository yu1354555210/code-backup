<template>
	<view class="wrapper">
		...
		<cu-custom bgColor="bg-white" :isBack="true"><block slot="backText"><view class="text-bold fontSize50">{{title}}</view></block></cu-custom>
...
export default {
	...
	methods: {
		...
		upCallback(page) {
			...
			let type = 1;
			let sign = md5(`${this.catId}${type}${pageNum}${pageSize}${this.signKey}`);
			uni.request({
				url: this.apiUrl + '/api/master/v1/cat_info',
				data: {
					sign: sign,
					cat_id: this.catId,
					type: type,
