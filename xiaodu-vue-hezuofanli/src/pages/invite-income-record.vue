<template>
  <div class="wrapper">
     <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
      <x-header :left-options="{backText: ''}">收支记录</x-header>
      <div class="content">
        <div class="invite-record-list card-item" id="dataList">
          <ul>
            <li v-for="(item,index) in dataList" :key="index">
              <div class="invite-record-fl">
                <div class="invite-record-title" v-text="item.title"></div>
                <div class="invite-record-other" v-text="item.create_time"></div>
              </div>
              <div class="invite-record-fr">
                <div v-if="item.type==2"><span class="minus">-{{item.amount}}</span>元</div>
                <div v-else><span class="add">+{{item.amount}}</span>元</div>
              </div>
            </li>
          </ul>
        </div>
    ...
    upCallback(page, mescroll) {
        this.$ajax
          ...
          .then(response => {
            ...
            let data = response.data.data.budget.data;
            // 如果是第一页需手动制空列表
