...
var lottery = {
	...
	// 开启抽奖
	_enable: function() {
		var isTel = jgg.el.isTel.val();
		var isCan = jgg.el.isCan.val();
		var syNum = jgg.el.syNum.val();
		var coin = Number(jgg.el.coin.text()); 
		var cjCoin = Number(jgg.el.cjCoin.val()); //抽奖消耗金币
		//是否符合抽奖条件
		if(isTel != 0 && isCan != 0 && syNum != 0 && coin >= cjCoin){
	...
	_stop: function() {
		...
		//还原抽奖按钮
		$(".cjbtn").removeClass('cur');
		$("#jggResultDialog").find('.dialog-info p').text($("#resultInfo").val()).parents('#jggResultDialog').fadeIn('fast');
	...
	beforeRoll: function() {
		var _this = lottery;
		// var coin = jgg.el.coin.text();
		// var cjCoin = jgg.el.cjCoin.val();
		_this._disable();
		if(_this.options.beforeRoll) {
			_this.options.beforeRoll.call(_this);
		}
		_this._roll();
		// jgg.el.coin.text(parseInt(coin - cjCoin));
	},
